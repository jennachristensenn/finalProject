...
Exploring summary stats of BMI (only numeric variable). Here the mean is very similar to the median indicating that the variable is not skewed, however these values are on the lower end compared to the max BMI value of 98. *Some speculation about the scale of these values...
```{r}
diab_data |>
...
Here we see the majority of BMI values fall between 10-50.
```{r}
ggplot(diab_data, aes(x = bmi)) +
  geom_histogram(aes(color = genHlth)) 
```


!!!!! include link to modeling page